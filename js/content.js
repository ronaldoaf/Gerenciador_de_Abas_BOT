	   ...
	   $(jogos).each(function(ii,jogo){			   
			 ...
			 if(  (ns(jogo.home)==ns(home)) && (ns(jogo.away)==ns(away)) ){
				   ...
				   jogo_selecionado=bot.jogoLive(home,away);
					...
					//Aposta no Home
						if (( ( jogo.ind>=CONFIG.ind1 ) &&  ( jogo.ind2>=CONFIG.ind2) && 	( jogo_selecionado.AH_Home>=0)  &&  ( jogo.gHm==0.0) && ( jogo.rH==0.0) &&  ( jogo.gHf-jogo.gAf<3) &&  ( (primeiroTempo() && (jogo_selecionado.tempo>=CONFIG.t1)) ||  (segundoTempo() && (jogo_selecionado.tempo>=CONFIG.t2))    ) )) { bot.lista_de_apostas.push(home+' v '+away);  bot.apostar(jogo_selecionado.selHome); }
					...
					//Aposta no Away
						   if (( ( jogo.ind<=-CONFIG.ind1 ) &&  ( jogo.ind2<=-CONFIG.ind2) && 	( jogo_selecionado.AH_Away>=0)  &&  ( jogo.gAm==0.0) && ( jogo.rA==0.0) &&  ( jogo.gAf-jogo.gHf<3) &&  ( (primeiroTempo() && (jogo_selecionado.tempo>=CONFIG.t1)) ||  (segundoTempo() && (jogo_selecionado.tempo>=CONFIG.t2))    ) ))  { bot.lista_de_apostas.push(home+' v '+away);  bot.apostar(jogo_selecionado.selAway); }
